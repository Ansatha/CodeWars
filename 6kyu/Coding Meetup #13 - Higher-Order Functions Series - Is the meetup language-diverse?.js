...
function isLanguageDiverse(list) {
    // The array keeps only the languages
    list = list.map(e => e.language);
    // Creating a 2nd array with uniq value languages
    let langCount = [...new Set(list)]
    // Transforming each language into times it is repeated
    .map(langSet => list
        .filter (langList => langSet == langList).length)
        // Returning true|false if maxLang/minLang <=2
        return Math.max(...langCount) / Math.min(...langCount) <= 2;
};
